...
/*   Updated: 2022/09/15 16:05:55 by mriant           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_find_here_name(char *s1, char name[17])
{
	int		i;
...
void	ft_close_here_space(int *temp_std, int *fd, char name[17],
	int *temp_exit)
{
	dup2(*temp_std, 0);
	close(*temp_std);
...
int	ft_here_doc(t_dlist *tokens, t_env **env, t_sig *new_sa)
{
	...
	ft_find_here_name("/tmp/.here_doc00", name);
